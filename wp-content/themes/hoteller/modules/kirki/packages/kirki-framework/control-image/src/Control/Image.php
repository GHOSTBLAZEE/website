...
class Image extends Base {
	...
	protected function content_template() {
		...
		<div class="image-wrapper attachment-media-view image-upload">
			...
			<# } else { #>
				<div class="placeholder"><?php esc_html_e( 'No image selected', 'hoteller' ); ?></div>
			<# } #>
			<div class="actions">
				<button class="button image-upload-remove-button<# if ( '' === url ) { #> hidden <# } #>"><?php esc_html_e( 'Remove', 'hoteller' ); ?></button>
				<# if ( data.default && '' !== data.default ) { #>
					<button type="button" class="button image-default-button"<# if ( data.default === data.value || ( ! _.isUndefined( data.value.url ) && data.default === data.value.url ) ) { #> style="display:none;"<# } #>><?php esc_html_e( 'Default', 'hoteller' ); ?></button>
				<# } #>
				<button type="button" class="button image-upload-button"><?php esc_html_e( 'Select image', 'hoteller' ); ?></button>
			</div>
		</div>
		<?php
	}
