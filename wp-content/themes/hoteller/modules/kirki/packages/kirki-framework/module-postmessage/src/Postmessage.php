		...
		if ( 'postMessage' === $args['transport'] ) {
			...
			if ( empty( $args['js_vars'] ) && ! empty( $args['output'] ) ) {
				...
				if ( isset( $args['output']['element'] ) ) {
					...
					_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'hoteller' ), esc_html( $args['settings'] ) ), '3.0.10' );
