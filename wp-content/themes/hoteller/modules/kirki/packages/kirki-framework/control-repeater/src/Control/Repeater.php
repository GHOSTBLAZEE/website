...
class Repeater extends Base {
	...
	public function __construct( $manager, $id, $args = [] ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'hoteller' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							[
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'hoteller' ),
								'option_none_value' => '0',
								'selected'          => '',
							]
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'hoteller' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'hoteller' ); ?>" >
									...
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'hoteller' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'hoteller' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'hoteller' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'hoteller' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'hoteller' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'hoteller' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'hoteller' ); ?>" >
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'hoteller' ); ?>
									<# } #>
								</figure>
								...
								<div class="actions">
									...
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'hoteller' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'hoteller' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'hoteller' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'hoteller' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'hoteller' ); ?></button>
				</div>
			</li>
		</script>
